...
function create_pop_sim
{
	...
	hyb=`echo "${final_formatted_indv}" | ./scripts/transpose.sh | awk -f ./scripts/hap_count.awk`  
	A_pop=`./scripts/transpose.sh tmp/${popA_scaffold_filename} | awk -f ./scripts/hap_count.awk`       
	B_pop=`./scripts/transpose.sh tmp/${popB_scaffold_filename} | awk -f ./scripts/hap_count.awk`       
	...
	A_pop_freq=`echo "1"$'\t'`                                                     
	B_pop_freq=`echo "1"$'\t'`                                                     
	A_pop_freq+=`awk -f ./scripts/final_update_hap_count.awk <(echo "${uniq_hap}") <(echo "${A_pop}")`
	B_pop_freq+=`awk -f ./scripts/final_update_hap_count.awk <(echo "${uniq_hap}") <(echo "${B_pop}")`
...
function create_lklhd
{
	...
	do
	    ...
	    ./mongrail2 -c tmp/${chrom_filename}  -A tmp/${A_freq_scaffold_filename} -B tmp/${B_freq_scaffold_filename} -i tmp/${hybrid_scaffold_sim_filename} -o tmp/${hybrid_scaffold_out_filename} &&
...
while getopts ':A:B:i:r:R:h' opt; do                                                         
	...
	h)
            echo -e "mongrail2 v1.0.0 mongrail2\nhttps://github.com/Mongrail-2-0/\n"
